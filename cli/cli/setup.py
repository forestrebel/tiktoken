    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}")) as progress:
        ...
        if not check_command("gh"):
            if not install_tool(
                "GitHub CLI",
                """
                curl -fsSL https://cli.github.com/packages/githubcli-archive-keyring.gpg | sudo dd ***REMOVED***.gpg &&
                echo "deb [arch=$(dpkg --print-architecture) ***REMOVED***.gpg] https://cli.github.com/packages stable main" | sudo tee /etc/apt/sources.list.d/github-cli.list > /dev/null &&
...
"""
        ...
        try:
            ...
            if not check_command("doctl"):
                if not install_tool(
                    "doctl",
                    "curl -sL https://github.***REMOVED***.101.0-linux-amd64.tar.gz | tar xz && sudo mv doctl /usr/local/bin"
                ):
                    return None
